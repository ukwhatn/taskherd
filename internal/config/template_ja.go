...
// promptTemplateJA is the built-in [session_start] prompt_template, injected by Default() and
// Load() rather than by SessionStart.TemplateFor() itself (see its doc comment for why).
const promptTemplateJA = `#{{id}} {{title}} に取り組んでほしい。
...
// fileContentJA is what config init writes in Japanese; it must describe the same settings as
// Default(). Environment specific values (GHES hosts, Jira site, address) stay commented out as
// examples.
const fileContentJA = `# taskherd 設定ファイル
