...
### board からの起動は board の外で走る

board の `g` は起動そのものを実行しない。`taskherd start <id> --cwd ... --prompt ...` を **board から切り離した別プロセスとして起こし、board は即座に閉じる**。resume（pane が消えたセッションの起こし直し）も同様に `taskherd jump <id> --session <uuid> --yes` へ渡す。
...
`taskherd board` を起動すると kanban ボードが開く。操作は GUI と同じ文法で統一してある: **移動＝矢印 / 決定＝Enter / 切替＝Tab / 削除＝Delete**。

閉じる操作は 2 つに分かれる。**入力欄を持たない画面は `q` で閉じ、`Esc` は入力欄・編集から抜けるときだけ使う**。`q` が文字入力になってしまう画面（追加モーダル・起動モーダル・詳細モーダルの編集中・picker）だけが `Esc` で閉じる。

| キー | 動作 |
|---|---|
| `←` `→` | 列フォーカスの移動 |
| `↑` `↓` | カードの移動 |
| `Tab` | 移行先ステータスのセレクタを開く（次の列が既定選択。`←→` で選び `Enter` で確定、`q` で閉じる） |
| `Enter` | 詳細モーダル |
| `Delete` / `Backspace` | タスク削除（`y`/`n` の確認あり） |
| `a` | タスク追加モーダル |
| `g` | 紐づくセッションへジャンプ（複数あれば `↑↓` で選択）。紐づいていなければ起動モーダルを開く。移動・起動したら board は閉じる |
| `r` / `R` | フォーカスカード / 全体のライブ再取得 |
| `t` | 完了・却下列（terminal 列）の折り畳み切替 |
| `q` / `Ctrl+C` | 終了（モーダルが開いていれば、まずそれを閉じる） |
...
- `q` で board へ戻る。ただし `link-pane` がセッション紐づけ済みタスクの詳細を直接開いた場合（前述）は、`q` で board へは戻らずポップアップごと終了する
- 項目の編集中だけは `Esc` で編集を取り消す（`q` はその場では文字として入る）
...
- `Enter` で起動する。**起動そのものは board の外で走る**（後述）。board はここで閉じるので、着地するのは起動したタブになる
- `Esc` でモーダルを閉じる
